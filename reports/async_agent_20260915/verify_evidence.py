...
def replay(bundle=None):
    ...
    assert backup["verified_files"] == manifest["files"] == 470
    assert backup["archives"] == manifest
    archive = HERE / "raw_records.tar.gz"
    assert sha(archive.read_bytes()) == manifest["compact"]["sha256"]
    ...
    files = read("async-files.json")["files"]
    ...
    assert restored_count == audit["states"] == 7976
    assert clip_count == audit["clips_checked"] == 6916
