"""Run the frozen H2 compute-budget learned-policy GPU holdout."""
...
EXPECTED_OUTPUT_BASENAME = "actionstream_transport_h2_budget_holdout_20260822"
EXPECTED_PROFILE = "fixed_0950_transport_h2_budget"
EXPECTED_STATES = [15, 16, 17, 18, 19]
EXPECTED_INTERVALS = {
    "actionstream_backend_pipelined_aligned": 1,
    "actionstream_backend_budgeted_pipelined_aligned": 5,
}
ALLOWED_RUNTIMES = set(EXPECTED_INTERVALS)
FROZEN_FILES = {
...
def _validate_protocol(path: Path) -> dict[str, Any]:
    ...
    if raw.get("hypothesis_id") != "transport_h2_compute_budget":
    ...
    if set(runtimes) != ALLOWED_RUNTIMES or len(runtimes) != 2:
        raise ValueError(f"Unexpected H2 runtimes: {runtimes}")
    if set(order) != ALLOWED_RUNTIMES or len(order) != 2:
        raise ValueError(f"Invalid H2 execution order: {order}")
    if raw.get("delay_profiles") != [
        {"key": EXPECTED_PROFILE, "kind": "fixed", "milliseconds": 950}
    ]:
        raise ValueError("H2 must contain only the namespaced fixed 950 ms profile")
    environment = raw.get("environment", {})
    if environment.get("initial_state_indices") != EXPECTED_STATES:
        raise ValueError("H2 scored reset identities changed")
    if raw.get("compact_matrix") != {
        "episodes_per_task": 5,
        "paired": True,
        "reuse_delay_trace_across_runtimes": True,
    }:
        raise ValueError("H2 compact matrix changed")
    measurement = raw.get("measurement_v2", {})
    if measurement.get("fresh_process_per_runtime") is not True:
        raise ValueError("H2 requires a fresh process per runtime and family")
    warmup = measurement.get("worker_warmup", {})
    if int(warmup.get("inference_calls", 0)) != 2:
        raise ValueError("H2 freezes exactly two actual-worker warmup calls")
    if int(warmup.get("initial_state_index", -1)) != 20:
        raise ValueError("H2 non-scored warmup reset changed")
    backend = raw.get("actionstream_backend", {})
    runtime_configs = {runtime: dict(backend.get(runtime, {})) for runtime in runtimes}
    for runtime, interval in EXPECTED_INTERVALS.items():
        config = runtime_configs[runtime]
        if config.pop("minimum_request_interval_steps", None) != interval:
            raise ValueError(f"H2 request interval changed for {runtime}")
    expected_common = {
        "latest_only_fallback": False,
        "delivery_scheduler_enabled": True,
    }
    if any(config != expected_common for config in runtime_configs.values()):
        raise ValueError("Request interval must be the only H2 runtime difference")
    ...
    if mismatches:
        raise RuntimeError(f"Frozen H2 candidate hash mismatch: {mismatches}")
    return raw
...
def _validate_cell(
    ...
    if len(records) != 5:
        raise RuntimeError(f"Expected five H2 rows in {cell_dir}")
    ...
    expected_interval = EXPECTED_INTERVALS[runtime]
    for index, row in enumerate(records):
        ...
        if engine_config.get("delivery_scheduler_enabled") is not True:
            raise RuntimeError(f"Delivery scheduler was not enabled in {cell_dir}")
        if engine_config.get("minimum_request_interval_steps") != expected_interval:
            raise RuntimeError(f"Request interval mismatch in {cell_dir}")
        if row.get("request_interval_steps") != expected_interval:
            raise RuntimeError(f"Recorded request interval mismatch in {cell_dir}")
    ...
    if (trace_count, telemetry_count, video_count) != (5, 5, 1):
        raise RuntimeError(
            f"Incomplete H2 artifacts in {cell_dir}: "
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(families) != 3 or len(set(families)) != 3:
        raise ValueError("H2 requires exactly three task families")
    if args.capture is not True:
        raise ValueError("H2 requires paired representative video capture")
    output_root = args.output_root.resolve()
    if output_root.name != EXPECTED_OUTPUT_BASENAME:
        raise ValueError(
            f"H2 output basename must be {EXPECTED_OUTPUT_BASENAME}, got {output_root.name}"
        )
    if output_root.exists():
        raise FileExistsError(f"One-shot H2 output already exists: {output_root}")
    ...
    if len(all_records) != 30:
        raise RuntimeError(f"Expected 30 H2 rows, got {len(all_records)}")
