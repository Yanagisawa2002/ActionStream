...
def audit(*, allow_missing_license: bool) -> tuple[dict, bool]:
    ...
    review_base = manifest["source"].get("review_base_commit")
    review_base_available = False
    review_paths: set[str] = set()
    if review_base:
        try:
            _git_lines("cat-file", "-e", f"{review_base}^{{commit}}")
            review_base_available = True
            review_paths.update(
                _git_lines("diff", "--name-only", f"{review_base}...HEAD")
            )
            review_paths.update(_git_lines("diff", "--name-only", "--cached"))
            review_paths.update(_git_lines("diff", "--name-only"))
    ...
    review_output_rows = [row for row in output_rows if row["path"] in review_paths]
    oversized = [
    ...
    technical_checks = {
        ...
        "review_base_available": review_base_available,
    ...
    report = {
        ...
        "tracked": {
            ...
            "review_outputs_files": len(review_output_rows),
            "review_outputs_bytes": sum(row["bytes"] for row in review_output_rows),
            "oversized": oversized,
