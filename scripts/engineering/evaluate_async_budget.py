...
from actionstream.llm_vla.async_native import IsolatedAsyncSimulationPort
...
PROTOCOL = ROOT / "configs/async_agent_acceptance_v2.json"
...
def historical_layouts():
    ...
    with tarfile.open(
        ROOT / "reports/async_agent_20260915/raw_records.tar.gz"
    ) as archive:
        for name in archive.getnames():
            if name.startswith(
                (
                    "async-acceptance-v2/normal/episode-",
                    "async-acceptance-v2/recovery/episode-",
                )
            ) and name.endswith("/initial_layout.json"):
                layouts.add(json.load(archive.extractfile(name))["sha256"])
    if len(layouts) != 142:
        raise ValueError("Previous async acceptance exclusion coverage changed")
    return layouts
...
def freeze(args):
    ...
    files += sorted((ROOT / "scripts/engineering").glob("*.py"))
    files += [ROOT / "reports/async_agent_20260915/raw_records.tar.gz"]
...
class FaultPort(IsolatedAsyncSimulationPort):
